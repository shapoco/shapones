...
#define CS_HIGH()   { gpio_put(SPIBUS_TFCARD_CS_PIN, 1 ); }
#define CS_LOW()    { gpio_put(SPIBUS_TFCARD_CS_PIN, 0 ); }
...
{
    spibus_dma_complete();
    spi_set_baudrate(spibus_inst, 10 * 1000 * 1000); 
    sleep_ms(1);
}

static BYTE rcvr_spi() {
    BYTE buff;
    spi_read_blocking(spibus_inst, 0xff, &buff, 1);
    return buff;
}
...
{
    spi_read_blocking(spibus_inst, 0xff, buff, btr);
}


static void xmit_spi(BYTE buff) {
    spi_write_blocking(spibus_inst, &buff, 1);
}
...
{
    spi_write_blocking(spibus_inst, buff, btx);
}
