...
static void apu_fill_buffer(pwm_audio::sample_t *buff);
...
void core0_main() {
  ...
  spibus_init();
  DSTATUS dsts = disk_initialize(0);
  if (dsts & STA_NOINIT) {
    SHAPONES_PRINTF("Failed to initialize disk (DSTATUS: 0x%02X)\n", (int)dsts);
  }
  ...
  apu_fill_buffer(pwm_audio::get_buffer(0));
  apu_fill_buffer(pwm_audio::get_buffer(1));
  pwm_audio::play();
  ...
  while (true) {
    ...
    if (display_refresh_req && !spibus_dma_is_busy()) {
...
void core1_main() {
  ...
  next_vsync_us = get_time_us();

  while (true) {
    ppu::status_t ppu_status;

    ppu::service(ppu_line_buff, skip_frame, &ppu_status);

    if (!skip_frame) {
      if (!!(ppu_status.timing & ppu::timing_t::END_OF_VISIBLE_LINE)) {
        convert_color(ppu_status.focus_y);
        if (ppu_status.focus_y == 10) {
          int32_t fps_cent = (int32_t)(ppu::get_fps() * 100);
          ::seg7::drawDec32(0, 0, fps_cent, 2);
        }
      }
...
static void apu_dma_handler() {
  pwm_audio::flip_buffer();
  apu_fill_buffer(pwm_audio::get_next_buffer());
}

static void apu_fill_buffer(pwm_audio::sample_t *buff) {
  apu::service(apu_buff, pwm_audio::LATENCY);
  for (int i = 0; i < pwm_audio::LATENCY; i++) {
    buff[i] = (uint32_t)apu_buff[i] << 16;
  }
}
