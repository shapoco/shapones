...
uint8_t frame_buffer[STRIDE * HEIGHT];
...
void refresh() {
  ...
  spibus_dma_write_start(frame_buffer, STRIDE * HEIGHT);
}

static void start_command(command_t cmd) {
  spibus_dma_complete();

  spi_set_baudrate(spibus_inst, 62'500'000);

  gpio_put(SPIBUS_DISPLAY_CS_PIN, 0);
  gpio_put(SPIBUS_DISPLAY_DC_PIN, 0);
  spi_write_blocking(spibus_inst, (const uint8_t *)&cmd, 1);
