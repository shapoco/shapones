...
static BYTE rcvr_spi() {
    BYTE buff;
    xiao_spi_read_blocking(0xff, &buff, 1);
    return buff;
}
...
{
    xiao_spi_read_blocking(0xff, buff, btr);
}


static void xmit_spi(BYTE buff) {
    xiao_spi_write_blocking(&buff, 1);
}
...
{
    xiao_spi_write_blocking(buff, btx);
}
...
{
    xiao_spi_dma_complete();
    xiao_spi_set_baudrate(10 * 1000 * 1000);
    xiao_sleep_ms(1);
